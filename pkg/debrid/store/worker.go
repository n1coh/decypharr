...
func (c *Cache) StartWorker(ctx context.Context) error {
	...
	// Schedule download link refresh job (only if interval is configured)
	if c.downloadLinksRefreshInterval != "" {
		...
		c.logger.Debug().Msg("Download link refresh disabled (WebDAV not enabled)")
	}

	// Schedule torrent refresh job (only if interval is configured)
	if c.torrentRefreshInterval != "" {
		...
		c.logger.Debug().Msg("Torrent refresh disabled (WebDAV not enabled)")
	}
